...
import 'package:flutter_sixvalley_ecommerce/view/screen/camp/camp.dart';
...
class HomePage extends StatelessWidget {
  ...
  Future<void> _loadData(BuildContext context, bool reload) async {
    await Provider.of<BannerProvider>(context, listen: false).getBannerList(reload);
    await Provider.of<CategoryProvider>(context, listen: false).getCategoryList(reload);
    //await Provider.of<FlashDealProvider>(context, listen: false).getMegaDealList(reload);
    await Provider.of<BrandProvider>(context, listen: false).getBrandList(reload);
    await Provider.of<ProductProvider>(context, listen: false).getLatestProductList('1', reload: reload);
  }
          ...
          child: CustomScrollView(
            ...
            slivers: [
              ...
              SliverAppBar(
                ...
                title: Image.asset(Images.splash_logo, height: 100, width:100,color: ColorResources.getPrimary(context)),
                actions: [
                  Padding(
                    ...
                    child:Row(
                      children: [
                        GestureDetector(
                          child:Text(getTranslated('face_shape', context) ,style: titilliumRegular.copyWith(
                          ...
                          onTap: (){
                            Navigator.push(
                                ...
                                MaterialPageRoute(builder: (_) => WebViewScreen(
                                  title: getTranslated('face_shape', context),
                                  url: AppConstants.FACE_SHAPE,
                                ),
                                ));
                          },
                        ),
                        SizedBox(width: 15.0,),
                        GestureDetector(
                          child:Text(getTranslated('camp', context) ,style: titilliumRegular.copyWith(
                              color: ColorResources.getPrimary(context),
                              fontSize: Dimensions.FONT_SIZE_DEFAULT)),
                          onTap: (){
                            Navigator.push(
                                context,
                             MaterialPageRoute(builder: (_) => WebViewScreen(
                              title: getTranslated('camp', context),
                            url: AppConstants.CAMP,
                            ),
                                ));
                          },
                        ),
                        ...
                        GestureDetector(
                          ...
                          child: Center(
                            child: Container(
                              child: Text(getTranslated('doctor_zone', context),
                    ...
                    children: <Widget>[
                      Padding(
                        ...
                        child:GestureDetectorText(
                          title:getTranslated('book_an_appointment', context) ,
                                      navigateTo: WebViewScreen(
                                        title: getTranslated('book_an_appointment', context),
                                        url:AppConstants.SLOT_AVAILABILITY ,
                                      ),
                        ),
                      ),
                      Padding(
                        padding:  EdgeInsets.all( Dimensions.PADDING_SIZE_EXTRA_SMALL),
                        child:GestureDetectorText(
                          title:getTranslated('our_clinic', context) ,
                                      navigateTo: WebViewScreen(
                                        title: getTranslated('our_clinic', context),
                                        url:AppConstants.OUR_ClINIC,
                                     ),
                        ),
                      ),
                      Padding(
...
class GestureDetectorText extends StatelessWidget
{
  ...
  final Widget navigateTo;
   ...
   GestureDetectorText(  { @required this.title,  @required  this.navigateTo, @required Function() onTap});
